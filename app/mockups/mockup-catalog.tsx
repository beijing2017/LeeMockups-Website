...
type Product = { sku: string; name: string; description: string; category: string; thumbnailPath: string; previewPath: string; purchaseProvider?: string; purchaseUrl?: string; deliveryUrl?: string; etsyUrl?: string; priceUsd?: number; resolution?: string; durationSeconds?: number; keywords?: string[]; assetVersion?: string };
...
function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="mockup-card">
    ...
    <div className="mockup-card-body"><div className="mockup-specs"><strong>{commerce.price}</strong><div><span>{commerce.resolution}</span><span>{commerce.duration}</span></div></div><div className="mockup-card-actions">{commerce.available ? <><a className="mockup-buy-button" href={commerce.purchaseUrl} target="_blank" rel="noreferrer">{commerce.buttonLabel}</a>{commerce.deliveryUrl && <a href={commerce.deliveryUrl}>Already purchased? Download →</a>}</> : <button className="mockup-buy-button unavailable" type="button" disabled>Coming Soon</button>}</div></div>
  </article>;
}
