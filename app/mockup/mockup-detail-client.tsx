...
export function MockupDetailClient({ sku, initialProduct }: { sku: string; initialProduct?: Product }) {
  ...
  const freeSample = useMemo(() => products.find((item) => item.isFree), [products]);
  useEffect(() => {
  ...
  const purchasePanel = <aside className="mockup-purchase-panel"><div className="mockup-purchase-meta"><div className="mockup-price"><strong>{product.isFree?"Free":commerce.price}</strong>{!product.isFree&&commerce.launchSpecial&&<span className="launch-badge">Launch Special</span>}</div><div><small>{commerce.resolution}</small><small>{commerce.duration}</small></div></div><small>Digital product · No physical item</small>{product.isFree?<a className="button primary purchase-primary purchased" href={`${assetBase}/free/${encodeURIComponent(product.sku)}`} target="_blank" rel="noreferrer"><Download size={17}/> Download free sample</a>:<PurchaseAction provider={purchaseProvider} providerProductId={providerProductId} purchaseUrl={commerce.purchaseUrl} sku={product.sku}/>}<Link className="mockup-client-download" href={productExperience.officialDownloadPath}><Download size={17}/><span><strong>Download Desktop App</strong><small>First-time use · Windows and macOS</small></span></Link><p>Review compatibility and the <Link href="/refunds/">refund policy</Link> before purchasing.</p>{commerce.deliveryUrl&&<a className="mockup-existing-download" href={commerce.deliveryUrl}>Already purchased? Download</a>}</aside>;
  return <>
    ...
    <section className="mockup-product-content shell"><div className="mockup-info-main">
      ...
      <section><span className="mockup-section-kicker">HOW IT WORKS</span><h2>Three simple steps</h2><ol className="mockup-steps"><li><b>1</b><div><strong>Install LeeMockups</strong><p>Download the free app for {product.supportedPlatforms || productExperience.supportedPlatforms}. Installation is only needed the first time.</p></div></li><li><b>2</b><div><strong>Open the template</strong><p>Open the downloaded <code>.mockup</code> file and add your PNG or JPG artwork.</p></div></li><li><b>3</b><div><strong>Generate your files</strong><p>Click Generate to create the finished video and five still images. {productExperience.typicalGenerationTime}.</p></div></li></ol></section>
