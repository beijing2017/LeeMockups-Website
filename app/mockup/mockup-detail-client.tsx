...
export function MockupDetailClient({ sku, initialProduct }: { sku: string; initialProduct?: Product }) {
  ...
  const purchasePanel = <aside className="mockup-purchase-panel"><div className="mockup-purchase-meta"><div className="mockup-price"><strong>{product.isFree?"Free":commerce.price}</strong>{!product.isFree&&commerce.launchSpecial&&<span className="launch-badge">Launch Special</span>}</div><div><small>{commerce.resolution}</small><small>{commerce.duration}</small></div></div><small>Digital product · No physical item</small>{product.isFree?(product.sampleUrl?<a className="button primary" href={product.sampleUrl} target="_blank" rel="noreferrer"><Download size={17}/> Download free sample</a>:<button className="button primary" disabled>Free sample coming soon</button>):<PurchaseAction provider={purchaseProvider} providerProductId={providerProductId} purchaseUrl={commerce.purchaseUrl} sku={product.sku}/>}<Link className="mockup-client-download" href={productExperience.officialDownloadPath}><Download size={17}/><span><strong>Download Desktop App</strong><small>First-time use · Windows and macOS</small></span></Link><p>Review compatibility and the <Link href="/refunds/">refund policy</Link> before purchasing.</p>{commerce.deliveryUrl&&<a className="mockup-existing-download" href={commerce.deliveryUrl}>Already purchased? Download</a>}</aside>;
  return <>
